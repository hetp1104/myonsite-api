...
var URL_ARRAY = [
    ...
    {value: 'asset_readings_edit', label: 'Edit Asset Readings'},
...
//add this route on 21-02-2022 by Monika Patel
exports.terms = function (req, res) {
    res.render('terms', {
        title: 'MyOnSite',
        setting_data: setting_detail.provider_terms_and_condition
    });
}
// Controller function for user terms
exports.user_terms = async function (req, res) {
    try {
        // Fetch the setting data from database instead of relying on a global variable
        const setting_detail = await Settings.findOne().exec();
        
        if (!setting_detail || !setting_detail.user_terms_and_condition) {
            // Handle case where terms are not found
            return res.status(404).send('Terms and conditions not found');
        }
        
        res.render('user_terms', {
            title: 'MyOnSite - User Terms',
            setting_data: setting_detail.user_terms_and_condition
        });
    } catch (error) {
...
exports.login = function (req, res) {
    ...
    if (typeof req.session.userid !== "undefined") {
        return res.redirect('/today_requests');
    }
    ...
    utils.check_request_params_for_web(req.body, [], function (response) {
        if (response.success) {
            ...
            admin.find({})
                ...
                .then(() => {
                    // Render login page after admin check is completed
                    return res.render("admin_login", {
                        environment: process.env.NODE_ENV || 'development',
                        version: '1.4.4'  // You may want to load this from a config or package.json
                    });
                })
                .catch((err) => {
        ...
        utils.check_request_params_for_web(req.body, [], async function (response) {
            if (response.success) {
                try {
                    ...
                    settings.server_url = req.get('host');
                    await settings.save();
                    
                    var is_google_map_lic_key_expired = settings.is_google_map_lic_key_expired;
                    var countryname = settings.countryname;
...
exports.admin_list = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        utils.check_request_params_for_web(req.body, [], function (response) {
            if (response.success) {
                admin.countDocuments({}).then((admincount) => {
                    if (admincount == 0) {
...
exports.landing = function (req, res, next) {
    ...
    utils.check_request_params_for_web(req.body, [], function (response) {
            if (response.success) {
            Settings.countDocuments({}, function (err, set) {
                if (set == 0) {
...
exports.session_data = async function (req, res, next) {
    ...
    try {
        switch (type) {
            ...
            case 'admin':
                const payment_transaction_detail = await Payment_Transaction.findOne({});
                if (payment_transaction_detail && payment_transaction_detail.is_stop_system) {
                    res.json({ success: false });
                } else {
                    res.json({
                        success: true,
                        session_data: req.session.admin,
                        cookies: req.cookies.language
                    });
                }
                break;

            default:
                res.json({ success: false, message: 'Invalid session type' });
        }
    } catch (error) {
