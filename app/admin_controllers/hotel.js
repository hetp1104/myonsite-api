...
//             next: '',
//             pre: '',
//             is_public_demo: setting_detail.is_public_demo
//         };
...
//             let end_state = '';
//         } else {
//             const field = req.body.sort_item[0];
//             const order = parseInt(req.body.sort_item[1]);
//             const item = req.body.search_item;
//             const value = req.body.search_value;

//             sort[field] = order;
//             templateVars.sort_field = field;
//             templateVars.sort_order = order;
//             templateVars.search_item = item;
//             templateVars.search_value = value;
//             templateVars.filter_start_date = req.body.start_date;
//             templateVars.filter_end_date = req.body.end_date;

//             let start_date = req.body.start_date;
//             let end_state = req.body.end_date;
...
//                     end_date.setHours(11, 59, 59, 999);
//                 } else if (!end_state) {
...
//         if (hotelcount === 0) {
//             return res.render('hotel_list', templateVars);
//         }
...
//             const field = req.body.sort_item[0];
//             if (field === 'hotel_name') {
//                 options.sort = { hotel_name: parseInt(req.body.sort_item[1]) };
//             } else if (field === 'unique_id') {
//                 options.sort = { unique_id: parseInt(req.body.sort_item[1]) };
//             } else {
//                 options.sort = { email: parseInt(req.body.sort_item[1]) };
//             }
//         }

//         // Get paginated results
//         const hotellist = await Hotel.paginate(
//             { $and: [{ $or: [query1, query2, query3, query4, query5, query6] }, query] },
//             options
//         );
...
//             templateVars.detail = [];
//             templateVars.currentpage = hotellist.page;
//             templateVars.pages = hotellist.pages;
//             templateVars.next = page + 1;
//             templateVars.pre = page - 1;
//             return res.render('hotel_list', templateVars);
//         }

//         // Process each hotel
//         const processedHotels = await Promise.all(hotellist.docs.map(async (data) => {
//             const id = data._id;
//             const tripQuery = { user_type_id: id };

//             const [triptotal, completedtriptotal] = await Promise.all([
//                 Trip.countDocuments(tripQuery),
//                 Trip.countDocuments({ ...tripQuery, is_trip_completed: 1 })
//             ]);

//             return {
//                 ...data.toObject(),
//                 total_trip: triptotal,
//                 completed_trip: completedtriptotal
//             };
//         }));

//         // Update template variables with final data
//         templateVars.detail = processedHotels;
//         templateVars.currentpage = hotellist.page;
//         templateVars.pages = hotellist.pages;
//         templateVars.next = page + 1;
//         templateVars.pre = page - 1;
...
//         // Render final result
//         res.render('hotel_list', templateVars);
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        Hotel.find({$and: [{$or: [query1, query2, query3, query4, query5, query6]}, query]}).then((array) => { 
            ...
            array.forEach(function (data) {
                ...
                var query = {};
                query['user_type_id'] = id;
                Trip.countDocuments(query).then((triptotal) => { 
                    query['is_trip_completed'] = 1;
                    Trip.countDocuments(query).then((completedtriptotal) => { 
                        if (j == array.length) {
                            data.total_trip = triptotal;
                            data.completed_trip = completedtriptotal;
                            generate_excel(req, res, array, timezone_for_display_date)
            ...
            if (response) {
                ...
                Hotel.findOne({phone: req.body.phone}).then((response) => {
                    ...
                    if (response) {
                        ...
                        var hash = crypto.createHash('md5').update(password).digest('hex');
                        var hotelcount = 1;
                        Hotel.countDocuments({}).then((hotel_count) => {
