...
exports.list = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        if (request == 'today_requests')
        {
            ...
            var condition = {$match: {'schedule_start_time': {$gte: startdate, $lt: enddate}}};
        ...
        } else
        {
            ...
            var condition  = {$match: {'status': {$gte: 11}}}
        }        
        ...
        if (request == 'today_requests')
        {
            var trip_condition = {$match: {status: {$eq: 4}}}
        }
        else{
...
exports.list_all = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        // Trip.aggregate([condition, condition1, condition2, condition3, condition4, condition5, lookup, unwind,lookup1, unwind1,lookup2, unwind2, search, filter, count]).then((array) => { 
            Trip.aggregate([condition, condition1, condition2, condition3, condition4, condition5, lookup, unwind,lookup1,unwind1, lookup2,unwind2, lookup3,unwind3,lookup4,unwind4,  search, filter, count]).then((array) => { 
            
            if (!array || array.length == 0)
            {
                array = [];
                res.render('all_request_list', {detail: array, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment,common_utils:utils});
            } else
            {
                var pages = Math.ceil(array[0].total / number_of_rec);
                Trip.aggregate([condition, condition1, condition2, condition3, condition4, condition5, lookup, unwind,lookup1,unwind1, lookup2,unwind2, lookup3,unwind3,lookup4,unwind4, search, filter, sort, skip, limit]).then((array) => { 
                    res.render('all_request_list', {detail: array, scheduled_request_pre_start_minute: setting_detail.scheduled_request_pre_start_minute, 'current_page': page, 'pages': pages, 'next': next, 'pre': pre, moment: moment,common_utils:utils});
                    delete message;
                }, (err) => {
                    utils.error_response(err, res)
                });
