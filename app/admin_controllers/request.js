...
exports.list = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        if (req.body.search_item == undefined)
        {
            ...
            filter_end_date = '';
            payment = 2;
            status = 1000; //change status on 02/22/2023 by krina 
        ...
        console.log(filter)
        Trip.aggregate([condition, trip_condition, payment_condition, status_condition, lookup, unwind,lookup1,unwind1,lookup2, unwind2, lookup3, unwind3, unwind3,lookup4,unwind4, search, filter, count]).then((array) => { 
        // Trip.aggregate([condition, trip_condition, payment_condition, status_condition, lookup, unwind, lookup1, lookup2, unwind2, lookup3, unwind3,lookup4,unwind4, search, filter, count]).then((array) => { 
            ...
            if (!array || array.length == 0)
            {
                ...
                res.render('request_list', {detail: array, request: request, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment,common_utils:utils, total_count: total_count});
            } else
            {
                ...
                Trip.aggregate([condition, trip_condition, payment_condition, status_condition, lookup, unwind, lookup1, lookup2, unwind2, lookup3, unwind3, unwind3,lookup4,unwind4, search, filter, sort, skip, limit]).then((array) => { 
                    ...
                    res.render('request_list', {detail: array, timezone_for_display_date: setting_detail.timezone_for_display_date, request: request, 'current_page': page, 'pages': pages, 'next': next, 'pre': pre, moment: moment,common_utils:utils,  total_count: total_count});
                    delete message;
                }, (err) => {
...
exports.trip_map = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        // Make sure Google API key exists
        if (!setting_detail.web_app_google_key) {
            return res.render('error', {message: 'Google Maps API key is missing'});
        }
        ...
        Trip.findById(id).then((trips) => { 
            ...
            Trip_Location.findOne(query).then((locations) => { 
                // Add callback parameter to ensure proper API loading
                var url = "https://maps.googleapis.com/maps/api/js?key=" + setting_detail.web_app_google_key + "&libraries=places&callback=initMap";
                
                // Ensure timezone is never undefined
                var timezone = setting_detail.timezone_for_display_date || 'UTC';
                
                var renderData = {
                    'data': trips, 
                    timezone_for_display_date: timezone, 
                    'url': url, 
                    'user_name': user_name, 
                ...
                if (locations) {
                    renderData.trip_path_data = locations;
                }
                
                res.render('trip_map', renderData);
            }).catch(err => {
                console.error("Error fetching trip locations:", err);
                res.render('error', {message: 'Failed to load trip location data'});
            });
        }).catch(err => {
            console.error("Error fetching trip:", err);
            res.render('error', {message: 'Failed to load trip data'});
        });
    } else {
        res.redirect('/admin');
    }
...
exports.trip_detail = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        var id = req.body.id;
        var user_name = req.body.u_name || 'N/A';
        var provider_name = req.body.pr_name || 'N/A';
        var query = {};
        query['tripID'] = id;
        ...
        Trip.findOne({"_id": id})
            .then((trips) => { 
                ...
                Trip_Order.findOne({"trip_id": trips._id}).then((trip_order_detail) => {
                    ...
                    Trip_Location.findOne(query).then((locations) => { 
                        // Change callback to match the function name in your frontend
                        var url = "https://maps.googleapis.com/maps/api/js?key=" + setting_detail.web_app_google_key + "&libraries=places&callback=initMap";
                        
                        // Ensure timezone_for_display_date is never undefined
                        var timezone = setting_detail && setting_detail.timezone_for_display_date ? 
                                       setting_detail.timezone_for_display_date : 'UTC';
                        ...
                        res.render('trip_detail', renderData);
                    }).catch((err) => {
                        console.error("Error fetching trip locations:", err);
                        utils.error_response(err, res);
                    });
                }).catch((err) => {
                    console.error("Error fetching trip order:", err);
                    utils.error_response(err, res);
                });
            }).catch((err) => {
                console.error("Error fetching trip:", err);
...
exports.getSchedulesList = function (req, res, next) {
    ...
    // Trip.aggregate([condition, condition1, condition2, condition3, condition4, condition5, lookup, unwind,lookup1, unwind1,lookup2, unwind2, search, filter, count]).then((array) => { 
        ...
        } else
        {
            ...
            console.log(array[0].total);
            var pages = Math.ceil(array[0].total / number_of_rec);
            Trip.aggregate([status_condition, condition, condition1, condition2, condition3, condition4, condition5, lookup, unwind,lookup1,unwind1, lookup2,unwind2, lookup3,unwind3,lookup4,unwind4, search, filter,  skip, limit])
            .then((array) => { 
...
exports.getTodaySchedules  = function (req, res) {
    ...
    var matchTodaySchedules = {
         $match: {
            $or: [
                {
                    schedule_start_time: {
                        $gte: startOfDay, // Today's date or later
                        $lt: endOfDay // End of today
                    }
                },
                {
                    schedule_start_time: {
                        $gte: endOfDay // Future dates
                    }
                }
            ]
        }
    ...
    var projectFields = {
        $project: {
            ...
            patientphone: "$user_detail.phone",
            first_name: "$provider_detail.first_name",
            last_name: "$provider_detail.last_name",
            phone: "$provider_detail.phone",
            providerLocation: "$provider_detail.providerLocation",
            _id: "$provider_detail._id",
            testcode: { $arrayElemAt: ["$order_detail.order.orderlist.testcode", 0] },
            testname: { $arrayElemAt: ["$order_detail.order.orderlist.testname", 0] },
            source_address: "$source_address",
            specimenname: { $arrayElemAt: ["$order_detail.order.orderlist.specimens.specimenname", 0] },
            requsitionSysID: "$trip_detail.requsitionSysID" 
        }
    };
    var lookup = {
        $lookup: {
    ...
    var unwind3 = { $unwind: "$trip_detail" };
    // Add other lookup stages as needed
    console.log("Executing Aggregate Query");
    Trip.aggregate([lookup, unwind, lookup1, unwind1, lookup2, unwind2,lookup3,unwind3,matchTodaySchedules, projectFields /* Add other lookups here */]).then((array) => {
        if (!array || array.length == 0) {
            array = [];
        }

        // Calculate total_count based on the length of the result array
