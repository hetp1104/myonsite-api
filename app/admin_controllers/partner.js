            ...
            if (response) {
                ...
                Partner.findOne({phone: req.body.phone}).then((response) => { 
                    if (response) {
                        ...
                        Partner.countDocuments({}, function(error, partner_count){
...
async function initializeTwilio() {
    try {
        ...
        if (setting) {
            const { twilio_account_sid, twilio_auth_token, twilio_number } = setting;

            if (twilio_account_sid && twilio_auth_token && twilio_number) {
                const client = twilio(twilio_account_sid, twilio_auth_token, twilio_number);
                ...
                console.warn('Missing Twilio credentials in settings');
            }
        } else {
            console.warn('No settings found for Twilio configuration');
        }
    } catch (error) {
...
exports.list = function (req, res, next) {
    ...
    if (typeof req.session.partner != "undefined") {
        ...
        Provider.countDocuments({$and: [{$or: [query1, query2, query3, query4, query5, query6]}, query]}).then((provider_count) => { 
...
exports.partner_list = function (req, res, next) {
    ...
    if (typeof req.session.userid != 'undefined') {
        ...
        Partner.countDocuments({$and: [{$or: [query1, query2, query3, query4, query5, query6]}, query]}).then((partnercount) => { 
