...
// exports.service_types = async function (req, res, next) {
//     try {
//         if (typeof req.session.userid === 'undefined') {
//             return res.redirect('/admin');
//         }

//         // Get the count using modern countDocuments method
//         const type_count = await Type.countDocuments({});

//         if (type_count === 0) {
//             return res.render('service_type', { service_data: [] });
//         }

//         // Get all service types
//         const service = await Type.find({});

//         const TRIP_TYPES = [
//             {
//                 id: constant_json.TRIP_TYPE_VISITOR,
//                 name: config_json.TRIP_TYPE_VISITOR_STRING
//             },
//             {
//                 id: constant_json.TRIP_TYPE_NORMAL,
//                 name: config_json.TRIP_TYPE_NORMAL_STRING
//             }
//         ];

//         // Process service data
//         const service_data = service.map(service_detail => {
//             const matchingType = TRIP_TYPES.find(type => type.id === service_detail.service_type);
...
//         res.render('service_type', { service_data });

//     } catch (error) {
//         console.error('Error in service types:', error);
...
//         var condition = {$match: {'cityid': {$eq: new mongoose.Types.ObjectId(req.body.cityid)}}};
...
exports.check_type_available = function (req, res) {
    if (typeof req.session.userid != 'undefined') {
        ...
        var query = {};
        query['typename'] = typename;
        Type.countDocuments(query).then((type_list) => { 
            if (type_list != 0) {
                res.json(1);
            } else {
                res.json(0);
