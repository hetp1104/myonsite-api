...
exports.trip_earning = async function (req, res, next) {
    try {
        console.log('Debug - selected_country:', selected_country);
        console.log('Debug - selected_city:', selected_city);
        var array = [];
        var i = 0;
        ...
        // Make sure setting_detail is available
        ...
        if (selected_country && selected_country !== 'all') {
            try {
                const countryObjectId = new mongoose.Types.ObjectId(selected_country);
                country_filter["$match"]['country_id'] = {$eq: countryObjectId};
                city_list = await City.find({countryid: countryObjectId});
                
                if (selected_city && selected_city !== 'all') {
                    const cityObjectId = new mongoose.Types.ObjectId(selected_city);
                    city_filter["$match"]['city_id'] = {$eq: cityObjectId};
                }
            } catch (error) {
                throw {
                    name: 'ValidationError',
                    message: 'Invalid country or city ID format'
                };
            }
        }
        ...
        // Updated: Using await instead of callback
        ...
        if (typeof req.session.userid != 'undefined') {
            if (selected_country == null && country_list.length > 0) {
                selected_country = country_list[0]._id.toString();
            }

            // Updated: Using await instead of callback
            const country = await Country.findOne({_id: new mongoose.Types.ObjectId(selected_country)});
            ...
            // Updated: Using await instead of callback
            const city = await City.findOne({_id: selected_city});
            ...
            // Updated: Using await instead of callback
            const count_array = await Trip.aggregate([trip_filter, country_filter, city_filter, lookup, search, count]);
            ...
            var pages = Math.ceil(count_array[0].total / number_of_rec);

            // Updated: Using await instead of callback
            const array = await Trip.aggregate([trip_filter, country_filter, city_filter, lookup, search, sort, skip, limit]);
            ...
            // Updated: Using await instead of callback
            const trip_total = await Trip.aggregate([trip_filter, country_filter, city_filter, trip_group_condition_total]);
            ...
            // Process dates in array before sending to view
            const processed_array = array.map(data => {
                return {
                    ...data,
                    formatted_date: data.provider_trip_end_time ? 
                        moment(data.provider_trip_end_time)
                            .tz(timezone_for_display_date || 'UTC')
                            .format("DD MMM 'YY") : 
                        ''
                };
            });
            ...
            res.render('trip_earning', {
                detail: processed_array,
                timezone_for_display_date: timezone_for_display_date || 'UTC',
                'current_page': page,
                provider_weekly_analytic: provider_weekly_analytic_data,
        ...
        console.error('Trip earning error:', err);
        
        // Check if it's a Mongoose/MongoDB error
...
exports.generate_trip_earning_excel = function (req, res, next) {
    ...
    if (selected_country != 'all') {
        country_filter["$match"]['country_id'] = {$eq: new mongoose.Types.ObjectId(selected_country)};

        City.find({countryid: new mongoose.Types.ObjectId(selected_country)}, function (error, city) {
            city_list = city;
        })
        if (selected_city != 'all') {
            city_filter["$match"]['city_id'] = {$eq: new mongoose.Types.ObjectId(selected_city)};
        }
    }
    ...
    Country.find({}).then((country_list) => { 
        ...
        if (typeof req.session.userid != 'undefined') {
            ...
            Country.findOne({_id: new mongoose.Types.ObjectId(selected_country)}).then((country) => { 
                if (country)
                {
                    timezone = country.country_all_timezone[0];
                }
