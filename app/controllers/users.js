...
/*** 
 * post outbound data to api 
 * Crated By: Ripal Patel
 * Created Date: 28-Apr-2021
 * ***/
exports.post_outbound = function (req, res) {
    utils.check_request_params(req.body, [{ name: 'user_id', type: 'string' }], function (response) {
        if (response.success) {
            var trip_json = [];
            var outbound={};
            Trip.find({ user_id: req.body.user_id }).then((trips) => {
                if (trips) {
                    trip_json=trips;
                } else {
                    res.json({ success: false, error_code: error_message.ERROR_CODE_USER_DETAIL_NOT_FOUND });
            ...
            User.findOne({ _id: req.body.user_id }).then((user) => {
                if (user) {
                   outbound={ user: user, trip_list: trip_json};
                   postrequestbuilder(outbound,'yourpostURL');
                    res.json({ success: true, outbounddata: outbound });
                } else {
                    res.json({ success: false, error_code: error_message.ERROR_CODE_USER_DETAIL_NOT_FOUND });
                }
            });
...
postrequestbuilder=function(outbound_data,post_path){
    var bseurl=config.apibaseurl;
    var post_options = {
        host: 'hosturl',
        port: '80',
        path: '/'+post_path,
        ...
        headers: {
            ...
            'Content-Length': Buffer.byteLength(outbound_data)
        }
    };
  ...
  var post_req = http.request(post_options, function(res) {
    ...
    // post the data
    post_req.write(outbound_data);
    post_req.end();
}

/*** 
 * Get Patient User Detials
 * Crated By: Monika Patel
 * Created Date: 09-Feb-2022
 * ***/
 exports.get_patientuserdetail = function (req, res) {
    utils.check_request_params(req.body, [{ name: 'email', type: 'string' }], function (response) {
        if (response.success) {

            var email = req.body.email;
            var apipath = config.PathPrefix+'/Phlebotomist/GetPatientDeatilsByEmail';
            ...
            utils.httpRequest(options, data).then(response => {
                console.log('response------2651----')
                console.log(response)
                var data= JSON.parse(response);
                if(data && data.UserPatientID > 0){
                    User.findOne({ userSysID: data.UserPatientID }).then((patient) => {
                        if (patient) {
                            console.log('---update---')
                            var updateUser = {
                            ...
                            User.findOneAndUpdate({ userSysID: data.UserPatientID }, updateUser, {new: true}).then((userdata) => {
                                console.log('---2669---')
 ...
 * ***/
 ...
 exports.get_patient_appointments_list = function (req, res) {
    utils.check_request_params(req.body, [{ name: 'patientid', type: 'string' }, { name: 'type', type: 'string' }], function (response) {
        if (response.success) {

            // var jwt = utils.validate_token(req.body.token);
            // if (jwt) {
                var type = req.body.type
                var condition = { user_id: ObjectId(req.body.patientid), status: {$gte: 1} }
                if(type == 'today'){
                    ...
                    condition.schedule_start_time = { $gte: startdate }
                    condition.is_trip_cancelled = 0
                    condition.is_trip_cancelled_by_provider = 0
                    condition.is_trip_cancelled_by_user = 0
                    condition.is_schedule_trip = true
                    condition.status = { $gte: 3 }
                }else{
                    var enddateData = new Date(Date.now());
                    enddateData = enddateData.setHours(0, 0, 0, 0);
                    enddate = new Date(enddateData);
                    condition.schedule_start_time = { $lt: enddate }
                }
                ...
                // })
                // .populate({
                //     path: 'current_provider',
                //     model: 'Provider'
                // })
                console.log(trip_condition)
                Trip.aggregate([trip_condition, lookup, lookup1, lookup2, selectLookup])
                .sort({ schedule_start_time: -1 })
                .then(async (response) => {
                    ...
                    if (response.length > 0) {
                        ...
                        res.json({
                            success: false,
                            data: [],
                            error_code: error_message.ERROR_CODE_SCHEDULE_TRIP_NOT_FOUND_patient
                        });
                    }
                });
            // }
...
/*** 
 * Get patient's appointment detail
 * Crated By: Monika Patel
 * Created Date: 14-02-2022
 * ***/
 
 exports.get_patient_appointment_detail = function (req, res) {
    utils.check_request_params(req.body, [{ name: 'tripid', type: 'string' }], function (response) {
        if (response.success) {

            // var jwt = utils.validate_token(req.body.token);
            // if (jwt) {
                
                var condition = { _id: ObjectId(req.body.tripid)}
                var lookup = {
                    $lookup:
                            {
                                from: "trip_orders",
                ...
                console.log(trip_condition)
                Trip.aggregate([trip_condition, lookup, lookup1, lookup2])
