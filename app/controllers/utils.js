    ...
    try {
        Settings.findOne({}, function (err, setting) {
            ...
            let transporter = nodemailer.createTransport({
                ...
                auth: {
                    user:email, //"outboundfax@indralok.com", // generated ethereal user    //notify@myonsitehealthcare.com
                    pass:password, //"fYTxkK3kwzSc", // generated ethereal password     //8Fu*9uDb
                },
                ssl: true
            });
...
/*** 
 * Get XML Storage Location 
 * Crated By: Ketan Prajapati
 * Created Date: 20-Mar-2021
 * ***/
exports.getExportXmlFolderPath = function (req) {
    return 'xml_document';
}

/*** 
 * Save json response to Xml 
 * Crated By: Ripal patel
 * Created Date: 20-Mar-2021
 * ***/
var xml2js = require('xml2js');
exports.save_xml_response = function (json_responce, filename) {
var builder = new xml2js.Builder();
    var xml = builder.buildObject(json_responce);
    var newfilename = filename + '_' + utils.tokenGenerator(4) + '.xml';

    var xmlpath = './data/' + utils.getExportXmlFolderPath() + '/' + newfilename;

    fs.writeFile(xmlpath, xml, 'text', function (err) {
        if (err) {
            utils.logger('error',err.message);
        } else {
            
            utils.logger('message','xml file saved successfully');
        }
    });
}
 ...
 * Created Date: 20-Mar-2021
 * ***/
 exports.logger = function (log_type, message) {
  console.log(log_type,message);
 }



/*** 
 * Http request method with promice 
 * Crated By: Ripal patel
 * Created Date: 12-5-2021
 * Example : how to call
 *  var options = {
                    hostname: yourhosturl,
                    port: 80,
                    path: /apipath,
                    method: 'POST', method type er or post
                    headers: {
               ...
               utils.httpRequest(options, data).then(response =>  {
                    //get the response here and your logic for response    
...
exports.httpRequestPrint= function (params, postData) {
    var chunks = "";
    return new Promise(function(resolve, reject) {
        var req = https.request(params, (res) => {
            res.setEncoding('utf8');
...
exports.saveImagetos3browser = function (local_image_path, image_name, id) {
    var bf = new Buffer(100000);
    var file_new_path = utils.saveImageFolderPath(id) + image_name;
    var awspath=webconfig.S3BucketName+"/"+file_new_path;
    console.log(awspath);
    AWS.config.update({ accessKeyId: webconfig.S3Accesskey, secretAccessKey: webconfig.S3Secrete });
    fs.readFile(local_image_path, function (err, data) {
        var s3 = new AWS.S3();
        var base64data = new Buffer(data, 'binary');
        s3.putObject({
            Bucket: webconfig.S3BucketName,// setting_detail.aws_bucket_name,
            Key: file_new_path,
            Body: base64data,
            ACL: 'public-read'
        }, function (resp, data) {
...
/*** 
 * Common method to send notification using FCM for IOS and Android
 * Created By: Ripal Patel
 * Created Date: 4-May-2021
 * ***/
 const FCM = require('fcm-node')
exports.sendPushNotification = function (app_type, device_type, device_token, messageType, soundFileName, extraParam) {

    Settings.findOne({}, function (err, setting_data) {
        var provider_app_gcm_key = setting_data.android_provider_app_gcm_key;
        var user_app_gcm_key = setting_data.android_user_app_gcm_key;
        // console.log("PUSH NOTIFICATION");
        var regTokens = device_token;
        var sender_key;

        if (app_type == constant_json.PROVIDER_UNIQUE_NUMBER) {
            sender_key = provider_app_gcm_key;
        } else {
            sender_key = user_app_gcm_key;
        }
...
/*** 
 * Rename this method from "sendPushNotification" to "sendPushNotification_OLD"
 * Reason: Created Same methodsend notification using FCM, common send method for IOS and Android
 * Renamed By: Ripal Patel
 * Renamed Date: 4-May-2021
 * ***/
exports.sendPushNotification_OLD = function (app_type, device_type, device_token, messageCode, soundFileName, extraParam) {
...
exports.validate_token_old = function (req, res) {

    var token = req.token;

    var apipath = config.PathPrefix + '/Phlebotomist/ValidateToken';
    ...
    var postreq = https.request(options, postreq => {
        ...
        postreq.on('data', resdata => {
            var validatedata = JSON.parse(resdata);

            if (validatedata.isValidToken) {
                res.json({
                    success: true
                });
            }
...
exports.get_trip_text_from_status = function(provider_status_code){
    let statusRowBg = '';     
    let statusTripText = '';
                    
        if(provider_status_code == 9) { 
            statusRowBg="#18C7A5"
            statusTripText = config_json.title_trip_status_completed
        }
        else if(provider_status_code == 4) { 
            statusRowBg="#3792CB"//Waiting Collection
            statusTripText = config_json.title_trip_status_waiting_collection
        }
        else if(provider_status_code == 6) { 
            statusRowBg="#000000"
            statusTripText = config_json.title_trip_status_trip_started 
        }
        else if(provider_status_code == 0) { 
            statusRowBg="#f8f34a"//Eligibility Pending
            statusTripText = config_json.title_trip_status_eligibility_pending
        }
        else if(provider_status_code == 1) { 
            statusRowBg="#d10000"
            statusTripText = config_json.title_trip_status_insurance_rejected
        }
        else if(provider_status_code == 2) { 
            statusRowBg="#3D8C40"
            statusTripText = config_json.title_trip_status_pending_assignment
        }
        else if(provider_status_code == 3) { 
            statusRowBg="#D8A099"//changed
            statusTripText = config_json.title_trip_status_pending_schedule
        }
        else if(provider_status_code == 7) { 
            statusRowBg="#94912C"
            statusTripText = config_json.title_trip_status_hold
        }
        else if(provider_status_code == 8) { 
            statusRowBg="#F9F56E"
            statusTripText = config_json.title_trip_status_process
        }
        else if(provider_status_code == 11) { 
            statusRowBg="#FFA500"
            statusTripText = config_json.title_trip_status_waiting_specimen
        }
        else if(provider_status_code == 21) { 
            statusRowBg="#D4B95E" //changed
            statusTripText = config_json.title_trip_status_waiting_results
        }
        else if(provider_status_code == 31) { 
            statusRowBg="#FCFAB6"
            statusTripText = config_json.title_trip_status_inprogress
        }
        else if(provider_status_code == 41) { 
            statusRowBg="#868686" //changed
            statusTripText = config_json.title_trip_status_reporting
        }
        else if(provider_status_code == 61) { 
            statusRowBg="#4AF8F3"
            statusTripText = config_json.title_trip_status_reported
        }
        else if(provider_status_code == 101) { 
            statusRowBg="#C4B6B8" //changed
            statusTripText = config_json.title_trip_status_cancled
        }
        else{ 
            statusRowBg="#FEFDEC"
            statusTripText = config_json.title_trip_status_all
        }
        return ({ status_row_bg: statusRowBg, status_trip_text:statusTripText })
}

/*** 
 * Get Short Text (Input as string) 
 * Crated By: Mayursinh Zala
 * Created Date: 08-Dec-2021
 * ***/
exports.get_short_text = function(yourString){

    // var yourString = ""; //replace with your string.
    // var maxLength = 6 // maximum number of characters to extract
    var maxLength = Number(constant_json.SHORT_TEXT_CHARACTER_LIMIT)
    ...
    return ({ trimmed_string: trimmedString })
}