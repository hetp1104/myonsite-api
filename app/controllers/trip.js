...
const TripsDocument = require('../models/trips_document');
const EquipmentStatus = require('../models/equipment_status');
const AWS = require('aws-sdk');
const fs = require('fs');
const path = require('path');
const PatientAuditLog = require('../models/patientAuditLog');
                        ...
                        City.findOne({ _id: city_id }).then((city_detail) => {
                            if (city_detail) {
                                ...
                                var trip = new Trip({
                                    ...
                                    phid:tripData.phid,
                                    patient_type:tripData.patient_type,                                    
...
exports.drop_off = function (req, res) {
    console.log("Drop off api");
        ...
        if (response.success) {
            ...
            if (jwt) {
                ...
                Trip.find({
                    ...
                    is_trip_cancelled_by_user: 0,
                    is_schedule_trip: true,
                    status: {$eq: 21},//added by Mayursinh Zala to match the status with LIS for schedule trip
                }).sort({ schedule_start_time: 1 }).then(async (response) => {
                    var totalMiles = 0;
                    var ETA = 0;
                    var trips = [];
                    let completedTripCount = 0; //added by Mayursinh Zala to pass the updated count for completed trip on 08-12-2021
                        ...
                        response.forEach(function (trip) {
                            ...
                            if (trip.current_provider && new Date(trip.schedule_start_time).toISOString().slice(0, 10) == searchdate) {
                                promises.push(TripOrder.findOne({ trip_id: trip._id }).then((tripOrder) => {
                                    ...
                                    //var stats = [];
                                    //var orderids = [];
                                    //change below variable type int to double on 15-03-2022 by Monika Patel
                                    trip.total_distance = (trip.total_distance).toFixed(2)
                                    var order = [];
                                    ...
                                    var ResultPostingUrl = config.ResultPostingUrl + '?procedureCode=PTP&requisitionSysID=' + trip.requsitionSysID + '&token=@token';
                                    ...
                                    console.log(ResultPostingUrl);
                                    console.log(preCheck);
                                    console.log(postCheck);
                                    console.log(trip.requsitionSysID);
...
////////////////////////////////////
exports.completed_trips = function (req, res) {
    console.log("Completed Trip api");
    utils.check_request_params(req.body, [{ name: 'providerid', type: 'string' }, { name: 'date', type: 'string' }, { name: 'token', type: 'string' }], function (response) {
        if (response.success) {
            console.log(req.body.date);
            var jwt = utils.validate_token(req.body.token);
            ...
            if (jwt) {
                ...
                Trip.find({
                    ...
                    is_trip_cancelled_by_user: 0,
                    is_schedule_trip: true,
                    status: { $in: [ 41, 21 ] },//added by Het Patel to match the status with LIS for schedule trip
                }).sort({ schedule_start_time: 1 }).then(async (response) => {
                    var totalMiles = 0;
                    var ETA = 0;
                    var trips = [];
...
function sendErrorEmail(errorDetails) {
    ...
    const mailOptions = {
        from: email,
        to: 'krina.shah@myonsitehealthcare.com',
        subject: 'Admin Panel Error',
        text: `An error occurred: ${errorDetails}`,
        html: `<p>An error occurred:</p><p>${errorDetails}</p>`
    };
...
exports.import_trip = function (req, res) {
    if (req.header('Content-Type').endsWith("xml")) {
        ...
        console.log("--6351--Order Import Trip :: " + order.labrequisitionid[0]);
        ...
        TripOrder.findOne({ requisitionsysid: order.requisitionsysid[0] }).then(req_data => {
            ...
            if (!req_data) {
                const errorDetails = `Trip order with RequisitionSysID ${order.requisitionsysid[0]} not found.`;
            ...
            console.log('Parsed Trip Order:', tripOrder);

            User.findOne({ userSysID: tripOrder.order.patient.user_patient_id }).then(user_data => {
                if (!user_data) {
                    const errorDetails = `User with UserSysID ${tripOrder.order.patient.user_patient_id} not found.`;
                    sendErrorEmail(errorDetails);
                    return res.status(404).json({
                        success: false,
                ...
                Provider.findOne({ userSysID: tripOrder.phlebotomistid }).then(provider => {
                    if (!provider) {
                        const errorDetails = `Provider with UserSysID ${tripOrder.phlebotomistid} not found.`;
                        sendErrorEmail(errorDetails);
                        return res.status(404).json({
                            success: false,
                    ...
                    if (!tripOrder.service_type_id) {
                        tripOrder.service_type_id = "6596625d8d78bc2bdb1486e6";
                    }

                    Citytype.findOne({ _id: tripOrder.service_type_id }).then(citytype => {
                        if (!citytype) {
                            const errorDetails = `City type with ID ${tripOrder.service_type_id} not found.`;
                            sendErrorEmail(errorDetails);
                            return res.status(404).json({
                                success: false,
                        ...
                        exports.check_trip_inside_zone_queue(citytype.city_id, tripOrder.order.patient.address.latitude, tripOrder.order.patient.address.longitude, function (data) {
                            ...
                            Trip.findOne({ requsitionSysID: requisitionsysid, triptype: 'Draw' }).then(response => {
                                if (!response) {
                                    const errorDetails = `Trip with RequisitionSysID ${requisitionsysid} not found.`;
                                    sendErrorEmail(errorDetails);
                                    return res.status(404).json({
                                        success: false,
                                        error_code: error_message.ERROR_CODE_TRIP_NOT_FOUND
                                ...
                                response.requsitionSysID = tripOrder.requisitionsysid;
                                response.provider_id = tripOrder.provider_id;
                                response.labRequisitionID = tripOrder.labRequisitionID;
                                response.status = tripOrder.status;
                                response.is_schedule_trip = tripOrder.start_time ? true : false;
                                response.schedule_start_time = tripOrder.start_time || null;
                                response.server_start_time_for_schedule = tripOrder.start_time || null;
                                response.lis_created_time = tripOrder.lis_created_time;

                                response.save().then(() => {
                                    tripOrder.trip_id = response.id;
                                    tripOrder.save().then(() => {
                                        res.json({
                                            success: true,
                                        ...
                                        console.error('Error saving TripOrder:', err);
                                        sendErrorEmail(err.message);
                                        res.status(500).json({
                                            success: false,
                                            message: 'Failed to save TripOrder'
                                        });
                                    });
                                }).catch(err => {
                        ...
                        console.error('Error finding CityType:', err);
                        sendErrorEmail(err.message);
                        res.status(500).json({
                            success: false,
                            message: 'Error finding CityType'
                        });
                    });
                }).catch(err => {
            ...
            console.error('Error finding TripOrder:', err);
            sendErrorEmail(err.message);
            res.status(500).json({
                success: false,
                message: 'Error finding TripOrder'
            });
        });
    }
};
                    ...
                    if (citytype) {
                        ...
                        City.findOne({ _id: city_id }).then((city_detail) => {
                            if (city_detail) {
                                ...
                                //add below condition on 10-03-2022 by monika as not update is_trip_cancelled flag while cancelled from LIS
                                ...
                                Country.findOne({ _id: country_id }).then((country_data) => {
                                    ...
                                    Trip_Service.find({
                                        ...
                                        trip.save().then(() => {
                                            console.log('trip-------insert new-------7100------------')
...
function parse_import_trip_xml(order, req_data) {
    ...
    // Add DROP_OFF_LOCATION and LOCATION_ACCOUNT_NUMBER to tripOrder
    tripOrder.drop_off_location = headerInfo.drop_off_location ? headerInfo.drop_off_location[0] : "";
    tripOrder.location_account_number = headerInfo.location_account_number ? headerInfo.location_account_number[0] : "";
    tripOrder.fasting = orderInfo.fasting[0], // add this key on 03-03-2022 by Monika patel
    ...
    if(patient!="" && clinic!="" && physician!=""){
        console.log('patient.address------7253---------')
		...
		phid: patient.phiid[0], // Add PHIID field
                patient_type: patient.type[0], 
                address: {
            ...
            clinic: {
                ...
                code: clinic.code[0],
                name: clinic.name[0],
                // add below key on 03-03-2022 by Monika patel
                phoneNumber: clinic.phonenumber[0], 
                serviceBy: clinic.serviceby[0]
            },
            physician: {
                ...
                // add below key on 03-03-2022 by Monika patel
                phonenumber: physician.phonenumber[0], 
                city: physician.city[0],
                state: physician.state[0],
                zip: physician.zip[0],
                country: physician.country[0],
                defaultclinicname: physician.defaultclinicname[0]
            }
        };
    }
    else{
    ...
    console.log('orders-------7392------')
    console.log(orders)
    tripOrder.order.orderlist = [];
    //add below if condition on 10-03-2022 by Monika patel
    if(orders != undefined){
    ...
    console.log(' @@@@@@@@@@@ xml return data on 7517 @@@@@@@@@@@@')
    console.log(tripOrder)
    return tripOrder;
};
...
exports.update_checklist = function (req, res) {
    utils.check_request_params(req.body, [
        ...
        if (response.success) {
            ...
            if (jwt) {
                ...
                // Custom validation for the supplies array
                if (!Array.isArray(supplies)) {
                    return res.json({
                        success: false,
                        error_code: error_message.ERROR_CODE_INVALID_PARAMETERS,
                        message: 'Supplies should be an array'
                    });
                }

                // Validate each supply item
                for (let supply of supplies) {
                    if (typeof supply.provider_id !== 'string' ||
                        typeof supply.supplies_name !== 'string' ||
                        typeof supply.checked !== 'boolean') {
                        return res.json({
                            success: false,
                            error_code: error_message.ERROR_CODE_INVALID_PARAMETERS,
                            message: 'Invalid supply item format',
                            supply
                        });
                    }

                    // Validate optional fields if they are present
                    if (supply.sysid && typeof supply.sysid !== 'string') {
                        return res.json({
                            success: false,
                            error_code: error_message.ERROR_CODE_INVALID_PARAMETERS,
                            message: 'Invalid sysid format',
                            supply
                        });
                    }

                    if (supply.trip_id && typeof supply.trip_id !== 'string') {
                        return res.json({
                            success: false,
                            error_code: error_message.ERROR_CODE_INVALID_PARAMETERS,
                            message: 'Invalid trip_id format',
                            supply
                        });
                    }
                }

                // Proceed with updating checklist items
                var updates = supplies.map(supply => {
                    return new Promise((resolve, reject) => {
                        let query = {
                        ...
                        // Add optional fields to query if they are provided
                        if (supply.sysid) {
                            query.sysid = supply.sysid;
                        }
                        if (supply.trip_id) {
                            query.trip_id = supply.trip_id;
                        }

                        EquipmentStatus.findOneAndUpdate(
                            query,
                            { $set: { checked: supply.checked, DTM: new Date() } },
                            { new: true, upsert: true },
                            function (err, item) {
                                if (err) {
                                    return reject({
                                        success: false,
                                        error_code: error_message.ERROR_CODE_DATABASE_ERROR,
                                ...
                                // Remove sysid and trip_id from the item before resolving
                                let updatedItem = item.toObject();
                                delete updatedItem.sysid;
                                delete updatedItem.trip_id;
...
exports.get_equipment_list = function (req, res) {
    utils.check_request_params(req.body, [
        ...
        if (response.success) {
            ...
            if (jwt) {
                var token = req.body.token;
                var schedule_time = new Date(req.body.schedule_time);
                ...
                utils.httpRequest(options, data).then(response => {
                    ...
                    if (data.success) {
                        // Retrieve the updated checklist items from the database for the given schedule date
                        var startOfDay = new Date(schedule_time);
                        startOfDay.setHours(0, 0, 0, 0);

                        var endOfDay = new Date(schedule_time);
                        endOfDay.setHours(23, 59, 59, 999);

                        EquipmentStatus.find({ DTM: { $gte: startOfDay, $lte: endOfDay } }, function (err, items) {
                            if (err) {
                                return res.json({
                                    success: false,
                                    error_code: error_message.ERROR_CODE_DATABASE_ERROR,
                                    message: 'Error retrieving the equipment list',
                                    error: err
                                });
                            }

                            // Create a map of checked items for quick lookup
                            var checkedMap = new Map();
                            items.forEach(item => {
                                checkedMap.set(item.supplies_name, item.checked);
                            });

                            // Update the IsChecked status based on the database records
                            data.lstItems.forEach(item => {
                                if (checkedMap.has(item.Specimens)) {
                                    item.IsChecked = checkedMap.get(item.Specimens);
                                } else {
                                    item.IsChecked = false; // Ensure default value is false
                                }
                            });
...
exports.reschedule_trip = function (req, res) {
    utils.check_request_params(req.body, [{ name: 'tripId', type: 'string' }, { name: 'token', type: 'string' }, { name: 'schedule_time', type: 'string' }, { name: 'reqsysid', type: 'string' }, { name: 'notes', type: 'string' }], function (response) {
        ...
        if (response.success) {
                ...
                Trip.find({ _id: req.body.tripId }).then((response) => {
                    ...
                    if (response.length == 0) {
                        ...
                        utils.httpRequest(options, data).then(response => {
                            ...
                            console.log("reschedule_trip 7782 starts error catch")
                            // console.log(err);
            ...
            console.log("reschedule_trip 7801 else ")
            res.json({
                success: false,
                error_code: response.error_code,
                error_description: response.error_description
...
exports.add_req_note = function (req, res) {
     utils.check_request_params(req.body, [
        ...
        //{ name: 'attachmentName', type: 'string' }  // Add attachmentName parameter
    ], function (response) {
        if (response.success) {
            var jwt = utils.validate_token(req.body.token);
            ...
            if (jwt) {
                ...
                if (fileUrl) {
                    document.push({
                        fileName: attachmentName,  // Use the document type as the attachment name
                        filePath: fileUrl,
            ...
            res.json({
                success: false,
                error_code: response.error_code, error_description: response.error_description
            });
        }
    });
};
                        ...
                        if (provider) {
                            ...
                            var group = {
                                $project: {
                                    ...
                                    lis_created_time: "$lis_created_time" // Krina Shah - 9/4/2024 - Adding the lis_created_time field here
                                }
                            };
                                    ...
                                    array.forEach(function (trip) {
                                        promises.push(TripOrder.findOne({ trip_id: trip._id }).then((tripOrder) => {
                                            ...
                                            if (tripOrder) {
												...
												console.log(printRequisitionUrl);
												console.log(receiveSpecimenUrl);
												// HetP - 9/2/2022 - Result Entry												console.log(ResultPostingUrl);
												console.log(preCheck);
												console.log(postCheck);
												console.log(trip.requsitionSysID);
                                                //end 13/9/21
...
/*** 
 * Receive Specimen
 * Crated By: Het Patel & Saumya
 * Created Date: 11-21-2022
 * ***/



  exports.receive_specimen = function (req, res) {
    console.log(req.body);
    utils.check_request_params(req.body, [{ name: 'specimenuserid', type: 'string' }, { name: 'requisitionid', type: 'string' }, { name: 'phlebotomistid', type: 'string' }, { name: 'collectiondtm', type: 'string' }, { name: 'notes', type: 'string' },
    { name: 'receiveddtm', type: 'string' }, { name: 'scheduleddtm', type: 'string' }, { name: 'usermail', type: 'string' },  { name: 'token', type: 'string' }], function (response) {
        if (response.success) {
console.log("receive_specimen");
console.log(req.body);
            var jwt = utils.validate_token(req.body.token);

            if (jwt) {
            Provider.findOne({ _id: req.body.phlebotomistid }).then((provider) => {
                if (provider) {

                    console.log(provider._doc.employee_id);

                var specimenlist = req.body.specimenuserid;
                var requisitionid = req.body.requisitionid;
                var phlebotomistid = provider._doc.employee_id;
                var collectiondtm = req.body.collectiondtm;
                var notes = req.body.notes;
                var receiveddtm = req.body.receiveddtm;
                var scheduleddtm = req.body.scheduleddtm;
                var autoprintspecimenlabels = "true";
                var printrequisitionlabel = "true";
                ...
                var apipath = config.PathPrefix + '/Requisition/ReceiveSpecimen';
                var data = JSON.stringify({
                    SpecimenListIDs: specimenlist,
                    RequisitionIDs: requisitionid,
                    Phlebotomists: phlebotomistid,
                    CollectionDtms: collectiondtm,
                    SpecNotes: notes,
                    ReceivedDtm: receiveddtm,
                    scheduledDtm: scheduleddtm,
                    AutoPrintSpecimenLabels: autoprintspecimenlabels,
                    PrintRequisitionLabel: printrequisitionlabel,
                    UserMail: usermail,
                    Token: token
                    // ReqSysId: reqsysid,
                    // UserEmail: '',
 ...
 * Created Date: 03-18-2024
 * ***/
exports.uploadTripDocument = function (req, res) {
    utils.check_request_params(req.body, [
        { name: 'trip_id', type: 'string' },
        { name: 'labRequisitionID', type: 'string' },
        { name: 'documentType', type: 'string' }
    ], function (response) {
        if (response.success) {
            // Find trip details
            Trip.findOne({ _id: req.body.trip_id }).then((tripDetail) => {
                if (tripDetail) {
                    // Find provider details from provider table using tripDetail.provider_id
                    Provider.findOne({ _id: tripDetail.provider_id }).then((providerDetail) => {
                        if (providerDetail) {
                            const employeeID = providerDetail.employee_id;
                            if (req.files && req.files.length > 0) {
                                ...
                                const bucketName = setting_detail.aws_bucket_name;
                                const folderName = 'MOSAPP';

                                const currentDate = new Date();
                                const formattedDate = `${currentDate.getFullYear()}${(currentDate.getMonth() + 1).toString().padStart(2, '0')}${currentDate.getDate().toString().padStart(2, '0')}`;
                                const dynamicFolder = `${folderName}/${employeeID}${formattedDate}`;

                                const image_name = tripDetail._id + utils.tokenGenerator(4);
                                const extension = path.extname(file.originalname);
                                const documentPath = '/var/www/html/newProdMyWork/src/data/trips_document';

                                let url;
                                if (req.body.documentType === 'collection') {
                                    url = path.join(documentPath, image_name + '_collection' + extension);
                                } else if (req.body.documentType === 'dropoff') {
                                    url = path.join(documentPath, image_name + '_dropoff' + extension);
                                } else if (req.body.documentType === 'drawstarted') {
                                    url = path.join(documentPath, image_name + '_drawstarted' + extension);
                                } else if (req.body.documentType === 'digital_signature') {
                                    url = path.join(documentPath, image_name + '_digital_signature' + extension);
                                } else {
                                    return res.json({ success: false, error_code: 'Invalid document type' });
                                }

                                fs.renameSync(file.path, url);

                                const s3Key = `${dynamicFolder}/${image_name}${extension}`;
                                const s3Params = {
                                    Bucket: bucketName,
                                    Key: s3Key,
                                ...
                                s3.upload(s3Params, (err, data) => {
                                    if (err) {
                                        console.error(err);
                                        return res.json({ success: false, error_code: 'Failed to upload file to S3.' });
                                    }

                                    const s3RelativePath = `${dynamicFolder}/${image_name}${extension}`;

                                    // Save the document information
                                    const tripsDocument = new TripsDocument({
                                        trip_id: req.body.trip_id,
                                        collection_document_picture: req.body.documentType === 'collection' ? s3RelativePath : '',
                                        dropoff_document_picture: req.body.documentType === 'dropoff' ? s3RelativePath : '',
                                        drawstarted_document_picture: req.body.documentType === 'drawstarted' ? s3RelativePath : '',
                                        digital_signature: req.body.documentType === 'digital_signature' ? s3RelativePath : '',
                                        labRequisitionID: req.body.labRequisitionID,
                                        documentType: req.body.documentType,
                                        is_uploaded: 1
                                    });

                                    tripsDocument.save().then(() => {
                                        tripDetail.is_document_uploaded = 1;
                                        tripDetail.save().then(() => {
                                            // Call the external API
                                            const formData = new FormData();
                                            formData.append('files', fs.createReadStream(url));

                                            axios.post('https://api.myonsitehealthcare.com/api/Requisition/UploadFile', formData, {
                                                params: {
                                                    RequisitionSysID: req.body.RequisitionSysID,
                                                    UserID: req.body.UserID,  // You may want to make this dynamic
                                                    Notes: 'File uploaded from trip document'  // You can customize this
                                                },
                                                headers: {
                                                    ...formData.getHeaders(),
                                                    'X-IndralokWebAPIKey': 'IndralokWebAPIAppId:IndralokWebAPIAppKey'
                                                }
                                            })
                                            .then(response => {
                                                res.json({
                                            ...
                                            .catch(error => {
                                                ...
                                                res.json({
                                                    success: false,
                                                    error_code: 'Failed to upload file to external API',
                                                    apiError: error.response ? error.response.data : error.message
                                                });
                                            });
                                        }).catch((err) => {
                                            console.error(err);
                                            res.json({ success: false, error_code: 'Failed to save trip details.' });
                                        });
                                    }).catch((err) => {
                                        console.error(err);
                                        res.json({ success: false, error_code: 'Failed to save document.' });
                                    });
                                });
                            } else {
                                res.json({ success: false, error_code: 'No file uploaded.' });
                            }
                        } else {
                            res.json({ success: false, error_code: 'Provider details not found.' });
                        }
                    }).catch((err) => {
                        console.error(err);
                        res.json({ success: false, error_code: 'Failed to fetch provider details.' });
                    });
                } else {
                    res.json({ success: false, error_code: 'Trip details not found.' });
                }
            }).catch((err) => {
                console.error(err);
                res.json({ success: false, error_code: 'Failed to fetch trip details.' });
            });
        } else {
            res.json({ success: false, error_code: response.error_code, error_description: response.error_description });
        }
    });
};
...
/*
Function added by Mayursinh zala to get the count of newtrip for the current provider 
...
exports.get_patient_data = function(req, res) {
    console.log("Received request body:", req.body);

    // Ensure SysID is present in the request body
	const SysID = req.body.SysID;

    if (!SysID) {
        console.log("Invalid or missing SysID:", SysID);
        return res.status(400).json({ success: false, message: "SysID is required", receivedBody: req.body });
    }

    // Construct the URL for the external API endpoint
    const url = 'https://qaapi.myonsitehealthcare.com/api/GetParticipantData';

    // Get the authentication token or key from config
    const authToken = config.AuthHeaderValue;

     // Make the POST request to fetch participant data with authentication headers
    axios.post(url, { SysID: SysID }, {
        headers: {
            'Content-Type': 'application/json',
            'X-IndralokWebAPIKey': authToken
        }
    })
    .then(response => {
        // Check if response contains participant data
        if (response.data && response.data.PatientSysID) {
            // Return the entire response data
            return res.status(200).json({ success: true, participant_data: response.data });
        } else {
            return res.status(404).json({ success: false, message: "Participant data not found" });
        }
    })
    .catch(error => {
        // Handle error
        console.error("Error fetching participant data:", error.message);
        return res.status(500).json({ success: false, message: "Failed to fetch participant data", error: error.message });
    });
}
    ...
    utils.check_request_params(req.body, [
        ...
        if (response.success) {
            ...
            if (!verificationCheckbox) {
                return res.json({ success: false, error_code: 'VerificationFailed', error_description: 'Please verify before updating.' });
            }

            // Fetch the trip and provider details
            const tripOrder = await TripOrder.findOne({ trip_id: trip_id });
            const provider = await Provider.findOne({ _id: tripOrder.provider_id });

            // Prepare update data
            const updateData = {
                "order.patient.firstname": name,
                "order.patient.gender": gender,
                "order.patient.dob": dateOfBirth,
                "order.patient.address.street": primaryAddress,
                "order.patient.secondaryAddress": secondaryAddress,
                "order.patient.home": home,
                "order.patient.work": work,
                "order.patient.mobile": mobile,
                "order.patient.other": other,
                "order.patient.primaryPhone": primaryPhone
            };

            try {
                // Log changes for each field being updated
                const changes = [
                    { field: "order.patient.secondaryAddress", oldValue: tripOrder.order.patient.secondaryAddress, newValue: secondaryAddress },
                    { field: "order.patient.home", oldValue: tripOrder.order.patient.home, newValue: home },
                    { field: "order.patient.work", oldValue: tripOrder.order.patient.work, newValue: work },
                    { field: "order.patient.mobile", oldValue: tripOrder.order.patient.mobile, newValue: mobile },
                    { field: "order.patient.other", oldValue: tripOrder.order.patient.other, newValue: other },
                    { field: "order.patient.primaryPhone", oldValue: tripOrder.order.patient.primaryPhone, newValue: primaryPhone }
                ];

                // Loop through changes and create audit logs if necessary
                for (const change of changes) {
                    if (
                        ...
                        const existingLog = await PatientAuditLog.findOne({
                            trip_id: trip_id,
                            provider_id: provider._id,
                        ...
                        if (!existingLog) {
                            const auditLog = new PatientAuditLog({
                                trip_id: trip_id,
                                provider_id: provider._id,
                                phlebotomist_id: provider._id,
                                field_name: change.field,
                                old_value: change.oldValue,
                                new_value: change.newValue,
                ...
                if (req.files && req.files.length > 0) {
                    ...
                    const bucketName = setting_detail.aws_bucket_name;
                    const folderName = 'patient_photos';
                    const image_name = `${trip_id}_${utils.tokenGenerator(4)}`;
                    const extension = path.extname(file.originalname);
                    const s3Key = `${folderName}/${image_name}${extension}`;
                    ...
                    updateData["order.patient.patientprofilepicture"] = s3Upload.Location;
                }

                // Update patient details in the database
                const updatedTripOrder = await TripOrder.findOneAndUpdate(
                    { trip_id: trip_id },
                    { $set: updateData },
                    { new: true }
                );
                ...
                if (!updatedTripOrder) {
                    return res.json({ success: false, error_code: 'TripOrderNotFound', error_description: 'Trip order not found.' });
                }

                res.json({
                    success: true,
                    message: "Patient details updated successfully",
                    data: updatedTripOrder
                });
            } catch (error) {
                console.error("Error updating patient details:", error);
                res.json({ success: false, error_code: 'UpdateFailed', error_description: 'Failed to update patient details.' });
            }
        } else {
            res.json({
                success: false,
