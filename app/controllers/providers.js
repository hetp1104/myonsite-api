...
const fs = require('fs')
// const LocationTracker = require('./location_tracker');
...
/**
 * Deleting log files from the server (Automated log file cleanup)
 * Created By: Krina Shah
 * Created Date: 05-Dec-2024
 */
// Directory where log files are stored
const logDirectory = '/var/www/html/newProdMyWork/src/log_files';
...
async function deleteLogFiles() {
  try {
    ...
    console.log('Log file cleanup completed.');
  } catch (error) {
    console.error('Error during log file cleanup:', error);
  }
}

// Schedule the function to run every 2 days at 5:00 AM
cron.schedule('0 5 */2 * *', async () => {
  console.log('Running scheduled log file cleanup...');
  await deleteLogFiles();
});

console.log('Log file cleanup job scheduled with node-cron.');
...
exports.provider_register = function (req, res, next) {
    console.log("in provider_register");
    ...
    { name: 'country', type: 'string' }], function (response) {
        // if (response.success) {
            ...
            if (true) {
                ...
                console.log(req.body); 
                if (provider) {
                    console.log("692 -- line")
                    console.log(req.body); 
                    if (provider.login_by == 'manual') {
                        console.log("695 --- line"); 
                        console.log(req.body); 
                        res.json({ success: false, error_code: error_message.ERROR_CODE_EMAIL_ID_ALREADY_REGISTERED });
                    } else {
                        console.log("in else 697 --- line");
                        console.log(req.body); 
                        res.json({
                            success: false,
                    ...
                    Provider.findOne({
                        ...
                        console.log("714 --line")
                        console.log(req.body); 
                        if (provider) {
                            res.json({ success: false, error_code: error_message.ERROR_CODE_PHONE_NUMBER_ALREADY_USED });
                        } else {
                            console.log("719 --line")
                            console.log(req.body); 
                            var query = {};
                            if (req.body.city_id) {
                                        ...
                                        if (country) {
                                            ...
                                            utils.insert_documets_for_new_providers(provider, 1, country._id, function (document_response) {
                                                ...
                                                provider.save().then(() => {
                                                    ...
                                                    response.street = provider.street;
                                                    response.street1 = provider.street1;
                                                    response.latitude = provider.latitude;
                                                    response.longitude = provider.longitude;
    ...
    // function insert_update_phlebotomist (req, res) {
       ...
       // providerid=id;
        ...
        Provider.findById(providerid).then((provider) =>{
          ...
          {
            var apipath = config.PathPrefix + '/Phlebotomist/InsertUpdatePhlebot t5omist';
            console.log("1071 --- line");
            console.log(req.body); 
                                            ...
                                            if (tripOrder) {
                                                ...
                                                if (tripOrder != null && tripOrder.order != null) {
                                                    ...
                                                    for (var j = 0; j < tripOrder.order.orderlist.length; j++) {
                                                        ...
                                                        trip.push({
                                                            trip: trip.order.orderlist.trip,
                                                        notes:trip.order.orderlist.notes,
                                                        precheck: trip.order.orderlist.preCheck,
                                                        postcheck: trip.order.orderlist.postCheck,
												        ResultPostingUrl:trip.order.orderlist.ResultPostingUrl,
                                                        faxUrl:trip.order.orderlist.faxUrl,
                                                        printRequisitionUrl:trip.order.orderlist.printRequisitionUrl,
                                                        receiveSpecimenUrl:trip.order.orderlist.receiveSpecimenUrl,
                                                        provider_id: trip.order.orderlist.provider_id,
                                                        order: trip.order.orderlist.order,
                                                        patient: trip.order.orderlist.patient,
                                                        physician:trip.order.orderlist.physician,
                                                        specimens: trip.order.orderlist.specimens,
                                                        results: trip.order.orderlist.results,
                                                        clinic: trip.order.orderlist.clinic,
                                                        totalMiles: trip.order.orderlist.totalMiles,
                                                        ETA: trip.order.orderlist.ETA,
                                                        status:trip.order.orderlist.status
...
exports.getProviderLocationAndImage = function (req, res) {
    // Check if userSysID is provided in the request body
    if (!req.body.userSysID || typeof req.body.userSysID !== 'string') {
        return res.status(400).json({
            success: false,
            error_code: 'Invalid or missing userSysID.',
            message: 'The userSysID is missing or invalid in the request body.'
        });
    }

    // If userSysID is valid, proceed with the rest of the logic
    Provider.findOne({ userSysID: req.body.userSysID }).then((providerDetail) => {
        if (providerDetail) {
            // Get latitude and longitude from providerLocation field
            const providerLocation = providerDetail.providerLocation || [];
            const latitude = providerLocation[0] || null;
            const longitude = providerLocation[1] || null;

            // Get profile image from picture field
            const profileImage = providerDetail.picture || null;

            // If either latitude/longitude or picture is missing, return error
            if (!latitude || !longitude) {
                return res.status(400).json({
                    success: false,
                    error_code: 'Location details missing.',
                    message: 'The provider location details (latitude or longitude) are missing.'
                });
            }

            // Return the provider data (latitude, longitude, and profile image) with 200 OK
            ...
            // Return 404 Not Found if no provider details found
            return res.status(404).json({
                success: false,
                error_code: 'Provider details not found.',
                message: 'No provider found for the given userSysID.'
            });
        }
    }).catch((err) => {
        console.error(err);
        // Return 500 Internal Server Error in case of a server issue
        return res.status(500).json({
            success: false,
            error_code: 'Failed to fetch provider details.',
            message: 'An unexpected error occurred while fetching provider details.'
        });
    });
};
...
exports.get_provider_setting_detail = function (req, res) {
    ...
    var user_terms_and_condition_url = req.protocol + '://' + req.get('host') + "/user_terms";  // User terms URL
    ...
    setting_response.user_terms_and_condition_url = user_terms_and_condition_url;  // Adding user terms URL
    ...
    Provider.findOne({ _id: provider_id }).then((provider_detail) => {
        ...
        if (provider_detail) {
            Country.findOne({ countryname: provider_detail.country }).then((country) => {
                ...
                response.ssn_no = provider_detail.ssn_no || "";
...
exports.get_user_terms_and_condition = function (req, res) {
    res.send(setting_detail.user_terms_and_condition); // Send user terms and conditions from settings
};

// Get User Privacy Policy
exports.get_user_privacy_policy = function (req, res) {
    res.send(setting_detail.user_privacy_policy); // Send user privacy policy from settings
};
    ...
    utils.check_request_params(req.body, [{ name: 'email', type: 'string' }], function (response) {
        if (response.success) {
            ...
            if (UserType != undefined && UserType == USER_TYPE_PATIENT_APP) {
                //this part of patient app login add this on 21-1-202
                var domain_apiurl = config.apiurl;
                console.log(domain_apiurl);
        ...
        Provider_Document.findById(req.body.id).then((provider_document) => {
            if (provider_document && provider_document.document_picture) {
                try {
                    ...
                    // Try direct download from the path stored in DB
                    res.download(provider_document.document_picture, path.basename(provider_document.document_picture), function(err) {
                        if (err) {
                            console.error('Error serving file:', err);
                            res.status(404).send('Document file not found. Please check the path.');
                        }
                    });
                } catch (error) {
