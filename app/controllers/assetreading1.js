    ...
    for (var i = 0; i < req.body.data.length; i++) {
        ...
        assetreadings.save()
            .then((devices_reading) => {
                ...
                axios.post('https://api.myonsitehealthcare.com/api/assetreadings/Insertassetreadings', jsonData, headers)
...
exports.provider_timeline = function (req, res) {
    // Assuming you have the provider ID in the request or you can get it from any source
    const providerID = req.params.providerID; // Change this to how you get the provider ID
    ...
    Provider.find({}, (err, providers) => {
        AssetReadingModel.find({ Value1: { $ne: null }, value2: { $ne: null } }, (err, assetReadings) => {
            ...
            // Fetch patient trip data using getTripData function for the specific provider
            getTripData(providerID, (err, tripData) => {
                if (err) {
                    // Handle the error, maybe render an error page or show a message to the user
                ...
                res.render('provider_timeline', {
                    ...
                    trips: tripData // Pass the fetched trip data to the view
                });
            });
        });
    });
